...
const STEPS: StoryStep[] = [
  ...
  { id:"quests", title:"DAILY QUESTS", map:"home", portrait:"/player_happy.png",
    ...
    detailMeta:[{toolSlot:98,navBtn:"tasks",targetPct:{x:NX.tasks,y:NY},dir:"down"},{toolSlot:0,targetPct:{x:NX.tasks,y:NY},dir:"down"},{toolSlot:4,targetPct:{x:NX.tasks,y:NY},dir:"down"},{toolSlot:-1,targetPct:{x:NX.tasks,y:NY},dir:"down"},{toolSlot:1,targetPct:{x:NX.tasks,y:NY},dir:"down"},{toolSlot:-1,targetPct:{x:NX.tasks,y:NY},dir:"down"},{toolSlot:-1,targetPct:{x:NX.nfts,y:NY},dir:"down"}] },
  { id:"wallet", title:"WALLET & WEB3", map:"home", portrait:"/player_idle.png",
    bubble:"Connect your wallet to save progress, claim LFG tokens, and get NFT boosts on Solana Devnet.",
    details:["Click the WALLET button (top-right) to open the wallet connection panel.","PHANTOM WALLET — Recommended Solana wallet. Install at phantom.app.","METAMASK — EVM wallet support for Ethereum users.","Once connected, your GOLD, EXP, level, and inventory are saved to the cloud.","INITIALIZE TOKEN ACCOUNT — Click once inside Wallet panel to set up your Solana token account.","CLAIM ALPHA NFT — Open MY NFTS panel and click CLAIM ALPHA NFT to receive LFG tokens on-chain.","NFT BOOST — Holding an Alpha NFT gives farming speed and harvest yield bonuses.","All blockchain interactions are on SOLANA DEVNET — test environment, no real money."],
    detailMeta:[{toolSlot:98,navBtn:"wallet",targetPct:{x:NX.wallet,y:NY},dir:"down"},{toolSlot:98,navBtn:"wallet",targetPct:{x:NX.wallet,y:NY},dir:"down"},{toolSlot:98,navBtn:"wallet",targetPct:{x:NX.wallet,y:NY},dir:"down"},{toolSlot:98,navBtn:"wallet",targetPct:{x:NX.wallet,y:NY},dir:"down"},{toolSlot:98,navBtn:"wallet",targetPct:{x:NX.wallet,y:NY},dir:"down"},{toolSlot:98,navBtn:"nfts",targetPct:{x:NX.nfts,y:NY},dir:"down"},{toolSlot:98,navBtn:"nfts",targetPct:{x:NX.nfts,y:NY},dir:"down"},{toolSlot:-1}] },
...
export default function PreFarmTutorial({ visible, onFinished, onMapFocus }: PreFarmTutorialProps) {
  ...
  return (
    <div ref={containerRef} style={{ position:"absolute", inset:0, zIndex:9500, overflow:"hidden", pointerEvents:"auto" }}>
      ...
      {/* Map selector replica */}
      <div className="map-sel-pft">
        {(["Farm","City","Fishing","Garden","Suburban"] as const).map((label,i)=>{
          const ids:MapType[]=["home","city","fishing","garden","suburban"];
          const isCur=ids[i]===step.map, isHl=activeSlot===99&&isCur;
          return <div key={label} className={`map-btn-pft ${isCur?"active":""} ${isHl?"hl":""}`}>{label.toUpperCase()}</div>;
        })}
      </div>
