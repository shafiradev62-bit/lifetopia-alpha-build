import { GameState, MapType, FARM_GRID, CROP_STAGES_COLORS, MAP_SIZES, CROP_GROW_TIMES, CROP_GOLD_REWARDS, MAP_COLLISIONS, MAP_PLAYER_START, SHOP_ITEMS, CollisionRect, Tree, GARDEN_ROAD_Y } from './Game';
...
export function loadImg(src: string): Promise<HTMLImageElement> {
  ...
  return new Promise((resolve, reject) => {
    const img = new Image();
    img.onload = () => { imgs[src] = img; resolve(img); };
...
export function preloadAssets() {
  return Promise.all([
    ...
    '/frame_1_berdiri.png', '/frame_2_jalan.png', '/frame_3_lari.png',
    '/frame_4_melompat.png', '/frame_5_duduk.png', '/frame_6_membungkuk.png',
    '/frame_7_merayap.png', '/frame_8_mengambil.png', '/frame_9_melempar.png',
    '/frame_10_menangkap.png', '/frame_11_berdiri.png', '/frame_12_jalan.png',
    '/farm_till.png', '/farm_plant.png', '/farm_water.png', '/farm_fertilize.png',
    '/farm_harvest.png', '/farm_weed.png', '/farm_check.png', '/farm_carry.png',
    '/player_walk1.png', '/player_walk2.png', '/jalan_kaki_10_tengah_langkah.png', '/jalan_kaki_11_akhir_langkah.png',
    '/jalan_kaki_1_berdiri_awal.png', '/jalan_kaki_12_siap_berhenti.png'
  ].map(loadImg));
}
...
function drawFarmPlots(ctx: CanvasRenderingContext2D, state: GameState) {
  ...
  for (const plot of state.farmPlots) {
    ...
    if (plot.tilled) {
      // Base soil color: even darker when watered as requested
      ctx.fillStyle = plot.watered ? '#1A0F06' : '#3D2510';
      ...
      if (plot.crop) {
        const prog = Math.min((Date.now() - plot.crop.plantedAt) / (plot.crop.growTime || 60000), 1);
        ...
        for (let c = 0; c < crackCount; c++) {
          ...
          ctx.beginPath();
          ctx.moveTo(sx, sy);
          ctx.lineTo(sx + ((seed * c * 5) % 10) - 5, sy + ((seed * c * 3) % 8) - 4);
          ctx.stroke();
        }
        // Small soil clumps
    ...
    if (plot.crop) {
      const { crop } = plot;
      const growTime = crop.growTime || 60000;
      const elapsed = Date.now() - crop.plantedAt;  // use real timestamp
      const prog = Math.min(elapsed / growTime, 1);

      let imgId = '';
      let emoji = '🌱';
      if (crop.type === 'wheat') { imgId = '/wheat.png'; emoji = '🌾'; }
      else if (crop.type === 'tomato') { imgId = '/tomato.png'; emoji = '🍅'; }
      else if (crop.type === 'carrot') { imgId = '/carrot.png'; emoji = '🥕'; }
      else if (crop.type === 'pumpkin') { imgId = '/pumpkin.png'; emoji = '🎃'; }
      
      const img = imgs[imgId];
      if (imgId && !imgs[imgId]) loadImg(imgId);

      const by = drawY + cellH - 4;
      const scales = [0.35, 0.6, 0.8, 1.0];
      const baseScale = scales[Math.min(crop.stage, 3)];
      ...
      if (crop.ready) {
        ...
        ctx.beginPath();
        ctx.ellipse(cx / shadowScaleX, (by + 2) / 0.3, 10 * scale, 4, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.restore();
      }
    ...
    // Holographic Label (Floating above plot)
    // Holographic Label (Floating above plot)
    ctx.font = 'bold 8px Arial';
    ctx.textAlign = 'center';
    let label = "";
    if (plot.crop) label = plot.crop.type.toUpperCase();
    else if (plot.tilled) label = "READY FOR SEEDS";

    if (label) {
      ctx.fillStyle = 'rgba(0,0,0,0.5)';
      ctx.fillText(label, plot.worldX + (cellW/2) + 1, plot.worldY + 11); // Shadow
      ctx.fillStyle = plot.crop ? '#FFD700' : '#FFF5E0';
      ctx.fillText(label, plot.worldX + (cellW/2), plot.worldY + 10);
    }
    ctx.restore();
  }
}
...
function drawTrees(ctx: CanvasRenderingContext2D, state: GameState) {
  ...
  for (const tree of state.trees) {
    ...
    if (tree.type === 'pine') {
      ...
      ctx.beginPath(); ctx.moveTo(0, -65); ctx.lineTo(30, -35); ctx.lineTo(-30, -35); ctx.closePath(); ctx.fill();
      ctx.beginPath(); ctx.moveTo(0, -50); ctx.lineTo(35, -20); ctx.lineTo(-35, -20); ctx.closePath(); ctx.fill();
      ctx.beginPath(); ctx.moveTo(0, -35); ctx.lineTo(40, -5); ctx.lineTo(-40, -5); ctx.closePath(); ctx.fill();
    } else {
      // Oak
      ctx.fillStyle = '#5D3A1A';
      ctx.fillRect(-8, -20, 16, 20);
      ctx.fillStyle = '#2D5A27';
      ctx.beginPath(); ctx.arc(0, -45, 28, 0, Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.arc(-15, -30, 22, 0, Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.arc(15, -30, 22, 0, Math.PI*2); ctx.fill();
    }
...
function drawFishingBobber(ctx: CanvasRenderingContext2D, state: GameState) {
  ...
  const bx = b.x, by = b.y + Math.sin(b.bobTimer)*5;
  ctx.save();
  ctx.strokeStyle = 'rgba(180,180,180,0.6)';
  ctx.lineWidth = 1.2;
  ctx.beginPath(); ctx.moveTo(state.player.x, state.player.y - 20); ctx.lineTo(bx, by); ctx.stroke();
  ...
  ctx.beginPath(); ctx.arc(bx, topY, 4, 0, Math.PI*2); ctx.fill();
...
function drawNPCs(ctx: CanvasRenderingContext2D, state: GameState) {
  for (let i = 0; i < state.npcs.length; i++) {
    ...
    const idleFeet = (timeScale / 600) % 2 < 1 ? '/jalan_kaki_1_berdiri_awal.png' : '/jalan_kaki_12_siap_berhenti.png';
    
    // VARIETY IN NPC BEHAVIOR (Mixing default idle with mood frames)
    const moods = [idleFeet, '/frame_004.png', idleFeet, '/frame_006.png', idleFeet, '/frame_007.png'];
    ...
    shadow.addColorStop(0, 'rgba(0,0,0,0.25)'); shadow.addColorStop(1, 'rgba(0,0,0,0)');
    ctx.fillStyle = shadow;
    ...
    ctx.beginPath(); ctx.roundRect(-22, -80, 44, 14, 4); ctx.fill();
    ctx.fillStyle = '#FFD700';
...
function drawFootprints(ctx: CanvasRenderingContext2D, state: GameState) {
  ...
  for (const fp of state.footprints) {
    ...
    const rot = fp.facing === 'right' ? Math.PI / 2
              : fp.facing === 'left'  ? -Math.PI / 2
              : fp.facing === 'up'    ? Math.PI
              : 0;
    ctx.rotate(rot);
...
function drawPlayer(ctx: CanvasRenderingContext2D, state: GameState) {
  const { x, y, facing, moving, action, actionTimer, running, targetX, targetY, jumpY } = state.player;
  
  const isWalking = (moving || (targetX !== null)) && !state.notification?.text.includes('LEVEL UP');
  ...
  if (state.notification?.text.includes('LEVEL UP')) {
    sprite = imgs[state.time % 1000 < 500 ? '/frame_011.png' : '/frame_012.png'];
  } else if (isJumping) {
    ...
    if (state.currentMap === 'home') {
      ...
      else if (action === 'axe' || action === 'celurit') sprite = imgs['/farm_weed.png'];
    ...
    const idleFeet = (state.time / 600) % 2 < 1 ? '/jalan_kaki_1_berdiri_awal.png' : '/jalan_kaki_12_siap_berhenti.png';
    const cycle = Math.floor(state.time / 2000) % 12;
    const moodMap: Record<number, string> = {
      0:  idleFeet, 1:  idleFeet,
      2:  '/frame_004.png', 3:  idleFeet,
      4:  '/frame_007.png', 5:  idleFeet,
      6:  '/frame_006.png', 7:  idleFeet,
      8:  idleFeet, 9:  idleFeet,
      10: idleFeet, 11: '/frame_005.png'
    };
    sprite = imgs[moodMap[cycle]] || imgs[idleFeet];
  }
  
  if (!sprite || !sprite.complete) sprite = imgs['/jalan_kaki_1_berdiri_awal.png'] || imgs['/chibi_1774349990714.png'];
...
function getToolImg(t: string | undefined): HTMLImageElement | null {
  if (!t) return null;
  if (t === 'axe') return imgs['/kapak_1774349990716.png'];
  if (t === 'celurit' || t === 'shovel' || t === 'hoe') return imgs['/celurit_1774349990712.png'];
  if (t === 'water') return imgs['/teko_siram.png'];
  if (t.includes('seed') || t === 'fertilizer' || t === 'karung') return imgs['/karung_1774349990717.png'];
  return null;
}
...
function drawVFX(ctx: CanvasRenderingContext2D, state: GameState) {
  ...
  for (const p of state.vfxParticles) {
    ...
    if (p.type === 'coin') {
      ...
      ctx.save(); ctx.translate(p.x,p.y); ctx.rotate(state.time/200+p.x);
      for (let i=0;i<4;i++){
        ctx.rotate(Math.PI/2);
        ctx.fillStyle=p.color;
        ctx.beginPath(); ctx.ellipse(0,-p.size*1.4,p.size*0.35,p.size*1.4,0,0,Math.PI*2); ctx.fill();
      }
      ctx.restore();
    } else if (p.type === 'drop') {
      ctx.save();
      ...
      ctx.beginPath(); ctx.arc(-s/3, -s/2, s/4, 0, Math.PI*2); ctx.fill();
      ctx.restore();
    } else if (p.type === 'dust') {
      const s = p.size;
...
function drawInstructionBubble(ctx: CanvasRenderingContext2D, state: GameState) {
...
function drawHologram(ctx: CanvasRenderingContext2D, cx: number, cy: number, text: string) {
  ctx.save();
  ctx.font = 'bold 8px "Press Start 2P", monospace';
...
function drawTooltip(ctx: CanvasRenderingContext2D, state: GameState) {
  const plotId = state.mouseHoveredPlotId;
  if (!plotId) return;
  const plot = state.farmPlots.find(p => p.id === plotId);
  if (!plot || !plot.crop || plot.crop.ready) return;

  const { cellW, cellH } = FARM_GRID;
  const cx = plot.worldX + cellW / 2;
  const cy = plot.worldY - 15;
  
  const now = Date.now();
  const elapsed = now - plot.crop.plantedAt;
  const growTime = plot.crop.growTime;
  const remaining = Math.max(0, Math.ceil((growTime - elapsed) / 1000));
  
  const text = `${plot.crop.type.toUpperCase()} - ${remaining} detik`;
